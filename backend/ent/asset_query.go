...
func (_q *AssetQuery) QueryUploads() *UploadQuery {
...
func (_q *AssetQuery) QueryProductImages() *ProductImageQuery {
...
func (_q *AssetQuery) WithUploads(opts ...func(*UploadQuery)) *AssetQuery {
	query := (&UploadClient{config: _q.config}).Query()
...
func (_q *AssetQuery) WithProductImages(opts ...func(*ProductImageQuery)) *AssetQuery {
	query := (&ProductImageClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *AssetQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Asset, error) {
	...
	if query := _q.withUploads; query != nil {
		if err := _q.loadUploads(ctx, query, nodes,
			func(n *Asset) { n.Edges.Uploads = []*Upload{} },
			func(n *Asset, e *Upload) { n.Edges.Uploads = append(n.Edges.Uploads, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withProductImages; query != nil {
		if err := _q.loadProductImages(ctx, query, nodes,
			func(n *Asset) { n.Edges.ProductImages = []*ProductImage{} },
			func(n *Asset, e *ProductImage) { n.Edges.ProductImages = append(n.Edges.ProductImages, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *AssetQuery) loadUploads(ctx context.Context, query *UploadQuery, nodes []*Asset, init func(*Asset), assign func(*Asset, *Upload)) error {
	...
	query.Where(predicate.Upload(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(asset.UploadsColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.asset_uploads
		if fk == nil {
			return fmt.Errorf(`foreign-key "asset_uploads" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "asset_uploads" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (_q *AssetQuery) loadProductImages(ctx context.Context, query *ProductImageQuery, nodes []*Asset, init func(*Asset), assign func(*Asset, *ProductImage)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*Asset)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.ProductImage(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(asset.ProductImagesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.asset_product_images
		if fk == nil {
			return fmt.Errorf(`foreign-key "asset_product_images" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "asset_product_images" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
