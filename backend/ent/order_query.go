...
func (_q *OrderQuery) QueryCustomer() *UserQuery {
...
func (_q *OrderQuery) QuerySeller() *SellerQuery {
...
func (_q *OrderQuery) QueryDriver() *EntregadorQuery {
	query := (&EntregadorClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *OrderQuery) QueryStatusHistory() *OrderStatusHistoryQuery {
	query := (&OrderStatusHistoryClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *OrderQuery) FirstID(ctx context.Context) (id string, err error) {
	var ids []string
...
func (_q *OrderQuery) FirstIDX(ctx context.Context) string {
...
func (_q *OrderQuery) OnlyID(ctx context.Context) (id string, err error) {
	var ids []string
...
func (_q *OrderQuery) OnlyIDX(ctx context.Context) string {
...
func (_q *OrderQuery) IDs(ctx context.Context) (ids []string, err error) {
...
func (_q *OrderQuery) IDsX(ctx context.Context) []string {
	ids, err := _q.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (_q *OrderQuery) WithCustomer(opts ...func(*UserQuery)) *OrderQuery {
	query := (&UserClient{config: _q.config}).Query()
...
func (_q *OrderQuery) WithSeller(opts ...func(*SellerQuery)) *OrderQuery {
	query := (&SellerClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *OrderQuery) WithDriver(opts ...func(*EntregadorQuery)) *OrderQuery {
	query := (&EntregadorClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *OrderQuery) WithStatusHistory(opts ...func(*OrderStatusHistoryQuery)) *OrderQuery {
	query := (&OrderStatusHistoryClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *OrderQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Order, error) {
	...
	if _q.withCustomer != nil || _q.withSeller != nil || _q.withDriver != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := _q.withDriver; query != nil {
		if err := _q.loadDriver(ctx, query, nodes, nil,
			func(n *Order, e *Entregador) { n.Edges.Driver = e }); err != nil {
			return nil, err
		}
	}
...
func (_q *OrderQuery) loadCustomer(ctx context.Context, query *UserQuery, nodes []*Order, init func(*Order), assign func(*Order, *User)) error {
...
func (_q *OrderQuery) loadSeller(ctx context.Context, query *SellerQuery, nodes []*Order, init func(*Order), assign func(*Order, *Seller)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*Order)
	for i := range nodes {
		if nodes[i].seller_orders == nil {
			continue
		}
		fk := *nodes[i].seller_orders
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "seller_orders" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *OrderQuery) loadDriver(ctx context.Context, query *EntregadorQuery, nodes []*Order, init func(*Order), assign func(*Order, *Entregador)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*Order)
	for i := range nodes {
		if nodes[i].entregador_orders == nil {
			continue
		}
		fk := *nodes[i].entregador_orders
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	query.Where(entregador.IDIn(ids...))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "entregador_orders" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *OrderQuery) loadStatusHistory(ctx context.Context, query *OrderStatusHistoryQuery, nodes []*Order, init func(*Order), assign func(*Order, *OrderStatusHistory)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*Order)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	for _, n := range neighbors {
		fk := n.order_status_history
		if fk == nil {
			return fmt.Errorf(`foreign-key "order_status_history" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "order_status_history" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (_q *OrderQuery) loadPayments(ctx context.Context, query *PaymentQuery, nodes []*Order, init func(*Order), assign func(*Order, *Payment)) error {
	...
	for _, n := range neighbors {
		fk := n.order_payments
		if fk == nil {
			return fmt.Errorf(`foreign-key "order_payments" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "order_payments" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
