...
func (_q *PaymentQuery) QueryOrder() *OrderQuery {
...
func (_q *PaymentQuery) FirstID(ctx context.Context) (id string, err error) {
	var ids []string
...
func (_q *PaymentQuery) FirstIDX(ctx context.Context) string {
...
func (_q *PaymentQuery) OnlyID(ctx context.Context) (id string, err error) {
	var ids []string
...
func (_q *PaymentQuery) OnlyIDX(ctx context.Context) string {
...
func (_q *PaymentQuery) IDs(ctx context.Context) (ids []string, err error) {
...
func (_q *PaymentQuery) IDsX(ctx context.Context) []string {
	ids, err := _q.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (_q *PaymentQuery) WithOrder(opts ...func(*OrderQuery)) *PaymentQuery {
	query := (&OrderClient{config: _q.config}).Query()
...
func (_q *PaymentQuery) loadOrder(ctx context.Context, query *OrderQuery, nodes []*Payment, init func(*Payment), assign func(*Payment, *Order)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*Payment)
	for i := range nodes {
		if nodes[i].order_payments == nil {
			continue
		}
		fk := *nodes[i].order_payments
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "order_payments" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
