...
func (_q *ProductQuery) QuerySeller() *SellerQuery {
...
func (_q *ProductQuery) QueryImages() *ProductImageQuery {
...
func (_q *ProductQuery) QueryInventory() *InventoryQuery {
...
func (_q *ProductQuery) QueryCartItems() *CartItemQuery {
	query := (&CartItemClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *ProductQuery) Clone() *ProductQuery {
	...
	return &ProductQuery{
		...
		withCartItems:  _q.withCartItems.Clone(),
...
func (_q *ProductQuery) WithSeller(opts ...func(*SellerQuery)) *ProductQuery {
	query := (&SellerClient{config: _q.config}).Query()
...
func (_q *ProductQuery) WithImages(opts ...func(*ProductImageQuery)) *ProductQuery {
	query := (&ProductImageClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *ProductQuery) WithInventory(opts ...func(*InventoryQuery)) *ProductQuery {
	query := (&InventoryClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *ProductQuery) WithCartItems(opts ...func(*CartItemQuery)) *ProductQuery {
	query := (&CartItemClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withCartItems = query
	return _q
}
...
func (_q *ProductQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Product, error) {
	var (
		...
		loadedTypes = [6]bool{
			...
			_q.withCartItems != nil,
		}
	)
	if _q.withSeller != nil || _q.withCategory != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := _q.withSeller; query != nil {
		if err := _q.loadSeller(ctx, query, nodes, nil,
			func(n *Product, e *Seller) { n.Edges.Seller = e }); err != nil {
			return nil, err
		}
	}
	...
	if query := _q.withInventory; query != nil {
		if err := _q.loadInventory(ctx, query, nodes,
			func(n *Product) { n.Edges.Inventory = []*Inventory{} },
			func(n *Product, e *Inventory) { n.Edges.Inventory = append(n.Edges.Inventory, e) }); err != nil {
			return nil, err
		}
	}
	...
	if query := _q.withCartItems; query != nil {
		if err := _q.loadCartItems(ctx, query, nodes,
...
func (_q *ProductQuery) loadSeller(ctx context.Context, query *SellerQuery, nodes []*Product, init func(*Product), assign func(*Product, *Seller)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*Product)
	for i := range nodes {
		if nodes[i].seller_products == nil {
			continue
		}
		fk := *nodes[i].seller_products
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "seller_products" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *ProductQuery) loadImages(ctx context.Context, query *ProductImageQuery, nodes []*Product, init func(*Product), assign func(*Product, *ProductImage)) error {
...
func (_q *ProductQuery) loadInventory(ctx context.Context, query *InventoryQuery, nodes []*Product, init func(*Product), assign func(*Product, *Inventory)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*Product)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.Inventory(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(product.InventoryColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.product_inventory
		if fk == nil {
			return fmt.Errorf(`foreign-key "product_inventory" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "product_inventory" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (_q *ProductQuery) loadCartItems(ctx context.Context, query *CartItemQuery, nodes []*Product, init func(*Product), assign func(*Product, *CartItem)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*Product)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
