...
// QuerySellerLinks chains the current query on the "seller_links" edge.
func (_q *UserQuery) QuerySellerLinks() *SellerUserQuery {
...
func (_q *UserQuery) QueryCart() *CartQuery {
...
// QueryDriverProfile chains the current query on the "driver_profile" edge.
func (_q *UserQuery) QueryDriverProfile() *EntregadorQuery {
...
// WithSellerLinks tells the query-builder to eager-load the nodes that are connected to
// the "seller_links" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UserQuery) WithSellerLinks(opts ...func(*SellerUserQuery)) *UserQuery {
	query := (&SellerUserClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *UserQuery) WithCart(opts ...func(*CartQuery)) *UserQuery {
	query := (&CartClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
// WithDriverProfile tells the query-builder to eager-load the nodes that are connected to
// the "driver_profile" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UserQuery) WithDriverProfile(opts ...func(*EntregadorQuery)) *UserQuery {
	query := (&EntregadorClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withDriverProfile = query
	return _q
}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := _q.withSellerLinks; query != nil {
	...
	if query := _q.withDriverProfile; query != nil {
		if err := _q.loadDriverProfile(ctx, query, nodes, nil,
			func(n *User, e *Entregador) { n.Edges.DriverProfile = e }); err != nil {
			return nil, err
		}
	}
...
func (_q *UserQuery) loadSellerLinks(ctx context.Context, query *SellerUserQuery, nodes []*User, init func(*User), assign func(*User, *SellerUser)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.SellerUser(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.SellerLinksColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.user_seller_links
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_seller_links" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
...
func (_q *UserQuery) loadCart(ctx context.Context, query *CartQuery, nodes []*User, init func(*User), assign func(*User, *Cart)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
...
func (_q *UserQuery) loadDriverProfile(ctx context.Context, query *EntregadorQuery, nodes []*User, init func(*User), assign func(*User, *Entregador)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.withFKs = true
	query.Where(predicate.Entregador(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.DriverProfileColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.user_driver_profile
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_driver_profile" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_driver_profile" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (_q *UserQuery) loadUploads(ctx context.Context, query *UploadQuery, nodes []*User, init func(*User), assign func(*User, *Upload)) error {
	...
	for _, n := range neighbors {
		fk := n.user_uploads
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_uploads" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_uploads" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
